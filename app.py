...
def init_supabase():
    ...
    try:
        # Try both st.secrets (Cloud) and os.getenv (Local)
...
def render_signal_panel(symbol, signal_data):
    ...
    if not signal_data or is_stale:
        ...
        # HTML FIX: Tutto a sinistra, niente spazi all'inizio delle righe
        st.markdown(f"""
...
""", unsafe_allow_html=True)
    ...
    # HTML FIX: Tutto a sinistra per evitare errori di rendering
    st.markdown(f"""
...
def main():
    ...
    # HTML FIX: Rimozione spazi iniziali anche qui
    ...
    for idx, symbol in enumerate(AppConfig.ASSETS):
        with tabs[idx]:
            ...
            with col_right:
                # HTML FIX: Rimozione spazi iniziali
                st.markdown(f"""
