...
def main():
    ...
    if analyze_button:
        ...
        # Nuova sezione: Consigli 3 Trade
        if show_predictions and 'prediction_result' in locals() and 'forecast' in locals():
            st.subheader("📈 Consigli 3 Trade")
           
            # Short Term
            short_dir = "Long" if prediction_result['prediction'] > 0 else "Short"
            short_prob = prediction_result['prob_up'] if short_dir == "Long" else prediction_result['prob_down']
            short_target = predicted_price
            short_stop = levels['support'][0] if short_dir == "Long" else levels['resistance'][0]
           
            # Medium Term
            medium_change = (target_1w - current_price) / current_price
            medium_dir = "Long" if medium_change > 0 else "Short"
            medium_prob = min(95, max(5, 50 + abs(medium_change * 100) * 5))
            medium_target = target_1w
            medium_stop = levels['support'][1] if medium_dir == "Long" else levels['resistance'][1]
           
            # Long Term
            long_change = (target_1m - current_price) / current_price
            long_dir = "Long" if long_change > 0 else "Short"
            long_prob = min(95, max(5, 50 + abs(long_change * 100) * 2))
            long_target = target_1m
            long_stop = levels['support'][2] if long_dir == "Long" else levels['resistance'][2]
           
            trade_df = pd.DataFrame({
                'Trade': ['Short Term', 'Medium Term', 'Long Term'],
                'Direzione': [short_dir, medium_dir, long_dir],
                'Entry': [current_price, current_price, current_price],
                'Target': [short_target, medium_target, long_target],
                'Stop Loss': [short_stop, medium_stop, long_stop],
                'Percentuale Successo': [short_prob, medium_prob, long_prob]
            })
            ...
            st.dataframe(
                trade_df.style.format({
                    ...
                    'Target': '${:,.2f}',
                    'Stop Loss': '${:,.2f}',
                    'Percentuale Successo': '{:.1f}%'
                }).background_gradient(subset=['Percentuale Successo'], cmap='RdYlGn'),
                use_container_width=True,
                hide_index=True
            )
