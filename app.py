...
# -*- coding: utf-8 -*-
"""
App.py — Streamlit Scenario & Analog Trading Dashboard (No Matplotlib Hard Dependency)
=====================================================================================
...
Key points
----------
- No mandatory matplotlib import (avoids ModuleNotFoundError on Streamlit Cloud).
- Uses Altair for charts if available; otherwise falls back to Streamlit native charts/tables.
- Interactive scenario inputs (DXY %, TNX bps, VIX pts), analog search, Ridge model, blended view,
  and long-only mean-variance portfolio suggestion.
...
def summarize_distribution(df: pd.DataFrame) -> pd.DataFrame:
    return pd.DataFrame({
        "mean": df.mean(),
        "median": df.median(),
        "p10": df.qualitative_quantile(0.10) if hasattr(df, "qualitative_quantile") else df.quantile(0.10),
...
# Select factor tickers (swap DXY with UUP proxy if requested)
...
if run_btn:
    try:
        ...
        # Try Altair boxplot, else show summary table
        if HAS_ALTAIR:
            # Melt analog returns to long form: columns=assets -> name,value pairs
            long_df = analog_rets.reset_index(drop=True).melt(var_name="Asset", value_name="Return")
            chart = (
                alt.Chart(long_df)
                .mark_boxplot(extent="min-max")
                .encode(x=alt.X("Return:Q", title="Return over Horizon"),
                        y=alt.Y("Asset:N", sort="-x"))
                .properties(height=220)
            )
            st.altair_chart(chart, use_container_width=True)
        else:
            st.info("Altair non disponibile: mostro riepilogo statistico al posto del boxplot.")
        st.dataframe(summarize_distribution(analog_rets).style.format("{:.2%}"), use_container_width=True)

        # Blended bar (Streamlit native)
        st.subheader("📌 Blended Predicted Forward Returns")
        st.bar_chart(blended.sort_values().to_frame("return"))
