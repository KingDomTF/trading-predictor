...
<style>
    * { font-family: 'Courier New', monospace; }
    ...
    .signal-box { background: #0a0a0a; border: 3px solid #00FF00; border-radius: 10px; padding: 1rem; margin: 1rem 0; }
</style>
""", unsafe_allow_html=True)

st.title("Elite Scalping AI ⚡")

# Sidebar settings
st.sidebar.title("Settings")
symbol = st.sidebar.text_input("Enter Symbol", value="EURUSD=X")
refresh_seconds = st.sidebar.slider("Refresh Interval (seconds)", min_value=5, max_value=60, value=5)
...
if symbol:
    ...
    if price_data:
        ...
        with col1:
            st.metric("Last Price", f"{price_data['last']:.5f}")
        
        with col2:
            st.metric("Bid/Ask", f"{price_data['bid']:.5f} / {price_data['ask']:.5f}")
        
        with col3:
            st.metric("Spread", f"{price_data['spread']:.5f} ({price_data['spread_pct']:.3f}%)")
        
        with col4:
            st.metric("Freshness", f"{price_data['freshness_seconds']:.1f}s")
        ...
        if signals:
            for setup in signals:
                ...
                st.write(f"**Stop Loss:** {setup['SL']}")
                st.write(f"**Take Profit:** {setup['TP']}")
                st.write(f"**Probability:** {setup['Probability']}%")
                st.write(f"**Filter Score:** {setup['Filter_Score']}%")
                st.write(f"**Signal Strength:** {setup['Signal_Strength']}")
                st.write(f"**ATR:** {setup['ATR']}")
                st.write(f"**Spread:** {setup['Spread']}")
                st.write("**Filters:**")
                for f in setup['Filters']:
                    st.write(f)
                st.markdown('</div>', unsafe_allow_html=True)
        else:
            st.info("No high-quality signals at the moment. Waiting for optimal setup...")
