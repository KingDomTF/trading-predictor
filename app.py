...
import datetime
import warnings
from pathlib import Path  # per percorso file MT4
...
# Simbolo MT4 -> ticker Yahoo (inverso)
...
def resolve_symbols(user_symbol: str):
    ...
    Gestisce:
      - GC=F, SI=F, BTC-USD, ^GSPC
      - XAUUSD, XAGUSD, BTCUSD, US500
      - altri simboli -> usati tali e quali.
    """
    user_symbol_clean = user_symbol.strip()

    # Prova match lato Yahoo (case-insensitive)
    ...
    # Prova match lato MT4 (case-insensitive)
    ...
    # Default: simbolo non mappato -> lo stesso per entrambi
    return user_symbol_clean, user_symbol_clean

# ⚠️ PERCORSO DEL FILE DI MT4
# Sostituisci TUTTO questo percorso con quello reale della tua cartella MQL4\Files.
# Esempio realistico (da adattare):
# r"C:\Users\TUO_UTENTE\AppData\Roaming\MetaQuotes\Terminal\XXXX_TERMINAL_ID\MQL4\Files\mt4_prices.csv"
...
def fetch_live_price(mt4_symbol: str):
    """
    Legge il prezzo live SOLO dal file mt4_prices.csv generato da MT4.

    mt4_symbol: simbolo come scritto dal broker (es. XAUUSD, BTCUSD, US500, XAGUSD).

    Ritorna:
      (last_price, prev_close)  oppure (None, None) se non trovato.
    """
    if "last_price_source" not in st.session_state:
        st.session_state["last_price_source"] = "N/D"

    mt4_symbol_clean = mt4_symbol.strip().upper()
    ...
    try:
        df = pd.read_csv(
            ...
            dtype={'symbol': str, 'time': str, 'last': float, 'prev_close': float},
        )
    ...
    if df.empty:
        st.session_state["last_price_source"] = f"MT4: file vuoto ({MT4_PRICES_FILE})"
        return None, None

    # Normalizza i simboli
    df["symbol"] = df["symbol"].astype(str).str.strip().str.upper()

    # Match diretto
    df_sym = df[df["symbol"] == mt4_symbol_clean]

    # Fallback: simboli che iniziano allo stesso modo (es. XAUUSD., XAUUSDm)
    if df_sym.empty:
        df_sym = df[df["symbol"].str.startswith(mt4_symbol_clean)]

    if df_sym.empty:
        st.session_state["last_price_source"] = (
            f"MT4: simbolo {mt4_symbol} non trovato in mt4_prices.csv"
        )
        return None, None

    # Prende l'ultima riga per quel simbolo
    row = df_sym.iloc[-1]
    last_price = float(row["last"])
    prev_close = float(row["prev_close"])

    st.session_state["last_price_source"] = f"MT4 ({row['symbol']})"
