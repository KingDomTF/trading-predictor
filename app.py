...
def calculate_scalping_setup(df_ind, model, scaler, current_price, bid, ask, spread):
    ...
    if prob_short > 65:
        ...
        setups.append({
            ...
            'SL': round(sl_short, 5),
            'TP': round(tp_short, 5),
            'Probability': round(prob_short, 1),
            'RR': round(rr_short, 2),
...
@st.cache_data(ttl=60)  # Cache 1 minuto per dati scalping
...
@st.cache_resource
def train_scalping_model_cached(symbol, interval='1m'):
    """Addestra modello scalping"""
...
if session_key not in st.session_state or refresh_btn:
